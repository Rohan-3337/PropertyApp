...
const TYPES = ["villa", "apartment", "house", "studio"] as const;
type PropertType = (typeof TYPES)[number];
...
const Create = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (priceNum > MAX_PRICE)
      return Alert.alert("Validation", `Price cannot exceed ₹${MAX_PRICE.toLocaleString("en-IN")}.`);
            ...
            <View className="flex-row flex-wrap gap-3">
              {form.localImages.map((uri, index) => (
                <View key={index} className="relative">
                  {/* ✅ Fixed: source={{ uri }} instead of src={uri} */}
                  <Image
                    source={{ uri }}
